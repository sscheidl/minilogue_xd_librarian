#!/usr/bin/env python3
"""
minilogue_xd_request_lab_v5.6_split_tx.py

Standalone diagnostic GUI for Korg minilogue xd SysEx request/read/write tests.

Scope:
- Reduced diagnostic GUI for current minilogue xd SysEx troubleshooting
- Auto-open/listen on all minilogue xd inputs and outputs
- Global Diagnostic 0x0E
- Current Program Request 0x10
- Slot via Program Change + Current Request
- Manual dump capture
- Load/save SysEx capture

Advanced/direct-slot/full-bank/write functions remain in code for later work,
but are intentionally not exposed in this simplified GUI.
...
v5.5 Port Buttons:
- keeps the simplified GUI but adds explicit diagnostic buttons for Slot via PC+Current:
  Port 1 only, Port 2 only, and Both ports
- RX still listens on all XD inputs, so each test isolates only the TX side

v5.6 Split TX:
- Program Change preparation now matches the XD's own outgoing sequence:
  CC0 Bank Select MSB = 0, CC32 Bank Select LSB = bank, Program Change = program
- Adds split diagnostic buttons: PC always goes to XD Port 2, then Current SysEx 0x10
  goes to Port 1 or Port 2, with or without trailing 00
- Wait after Program Change increased to 250 ms
...
def program_change_messages(slot_index: int, channel: int = 0) -> list[bytes]:
    """Bank Select MSB + LSB + Program Change before Current Program Request.

    The minilogue xd itself emits this three-message sequence when changing
    programs from the front panel:
    - CC 0  = Bank Select MSB, usually 0
    - CC 32 = Bank Select LSB, bank 0..4 for slots 001..500
    - Program Change, program 0..99 within the bank
    """
    bank, prog = slot_to_program_change(slot_index)
    ch = channel & 0x0F
    return [
        bytes([0xB0 | ch, 0x00, 0x00]),  # CC 0 Bank Select MSB
...
class RequestLab:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("minilogue xd Request Lab v5.5 - Port Buttons")
        ...
        self._build_gui()
        self._log("v5.6 Split TX: PC goes to Port 2; Current SysEx can be sent to Port 1/2 with or without trailing 00.")
        root.protocol("WM_DELETE_WINDOW", self.on_close)
        root.after(50, self.process_queue)
        root.after(500, self.auto_open_all_xd_startup)

    def _build_gui(self) -> None:
        """Build a deliberately reduced diagnostic GUI.

        Hidden/default settings:
        - TX mode: All XD outputs
        - SysEx channel/device ID: 0
        - Timeout: 3.0 s
        - Current trailing 00: ON
        - Direct 0x1C slot requests, probes, full-bank and write functions stay in code,
          but are not exposed in the simple GUI.
        """
        self.root.columnconfigure(0, weight=1)
        self.root.rowconfigure(3, weight=1)

        top = ttk.LabelFrame(self.root, text="MIDI")
        top.grid(row=0, column=0, sticky="ew", padx=8, pady=8)
        top.columnconfigure(4, weight=1)
        ttk.Button(top, text="Refresh", command=self.refresh_ports).grid(row=0, column=0, padx=4, pady=6)
        ttk.Button(top, text="Open all XD ports", command=self.open_all_xd_ports).grid(row=0, column=1, padx=4, pady=6)
        ttk.Button(top, text="Close", command=self.close_ports).grid(row=0, column=2, padx=4, pady=6)
        ttk.Label(top, text="Mode: listen on all XD inputs; choose TX by request button").grid(row=0, column=3, padx=10, pady=6, sticky="w")
        ttk.Label(top, textvariable=self.status_var).grid(row=1, column=0, columnspan=5, padx=4, pady=(0, 6), sticky="ew")

        req = ttk.LabelFrame(self.root, text="Diagnostic requests")
        ...
        ttk.Button(req, text="Global test 0x0E", command=self.request_global).grid(row=0, column=2, padx=4, pady=6)
        ttk.Button(req, text="Current 0x10", command=self.request_current).grid(row=0, column=3, padx=4, pady=6)
        ttk.Button(req, text="Split SX P1 10 00", command=self.request_split_p2_pc_p1_sysex_tz_on).grid(row=0, column=4, padx=4, pady=6)
        ttk.Button(req, text="Split SX P2 10 00", command=self.request_split_p2_pc_p2_sysex_tz_on).grid(row=0, column=5, padx=4, pady=6)
        ttk.Button(req, text="Split SX P1 10", command=self.request_split_p2_pc_p1_sysex_tz_off).grid(row=0, column=6, padx=4, pady=6)
        ttk.Button(req, text="Split SX P2 10", command=self.request_split_p2_pc_p2_sysex_tz_off).grid(row=0, column=7, padx=4, pady=6)
        ttk.Button(req, text="Listen manual dump", command=self.listen_manual).grid(row=0, column=8, padx=4, pady=6)
        ttk.Button(req, text="Cancel", command=self.cancel_operation).grid(row=0, column=9, padx=4, pady=6)

        files = ttk.LabelFrame(self.root, text="Capture")
        ...
        ttk.Label(files, textvariable=self.progress_var).grid(row=0, column=2, sticky="w", padx=8)
    ...
    def _target_output_names(self) -> list[str]:
        ...
        return self._target_output_names_for_mode(mode)

    def _target_output_names_for_mode(self, mode: str) -> list[str]:
        """Return open output targets for a named TX mode.

        Used by the explicit v5.5 buttons to isolate Port 1, Port 2, or Both
        without exposing a general TX-mode setting in the simplified GUI.
        """
    ...
    def request_split_p2_pc_p1_sysex_tz_on(self) -> None:
        self.request_slot_split_tx("XD Port 2", "XD Port 1", True)

    def request_split_p2_pc_p2_sysex_tz_on(self) -> None:
        self.request_slot_split_tx("XD Port 2", "XD Port 2", True)

    def request_split_p2_pc_p1_sysex_tz_off(self) -> None:
        self.request_slot_split_tx("XD Port 2", "XD Port 1", False)

    def request_split_p2_pc_p2_sysex_tz_off(self) -> None:
        self.request_slot_split_tx("XD Port 2", "XD Port 2", False)

    def request_slot_split_tx(self, pc_mode: str, sysex_mode: str, trailing_zero: bool) -> None:
        """Send CC0+CC32+Program Change on one port, then Current SysEx on another.

        Diagnostic v5.6 strategy:
        - normal MIDI program selection is known to work on XD Port 2
        - SysEx Current Request might need Port 1 or Port 2
        - trailing 00 is tested explicitly by separate buttons
        """
        try:
            slot = self.slot_index()
            ch = self.channel()
            pc_msgs = program_change_messages(slot, ch)
            sysex = request_current_program(ch, trailing_zero)
            pc_targets = self._target_output_names_for_mode(pc_mode)
            sysex_targets = self._target_output_names_for_mode(sysex_mode)
        except Exception as exc:
            messagebox.showerror("Split PC+Current", str(exc))
            return
        ...
        if not pc_targets:
            self._log(f"No PC TX targets open for mode={pc_mode!r}.")
            return
        if not sysex_targets:
            self._log(f"No SysEx TX targets open for mode={sysex_mode!r}.")
            return

        try:
            self._log(
                f"TX Split PC+Current slot {slot + 1:03d}: "
                f"PC mode={pc_mode!r}, SysEx mode={sysex_mode!r}, "
                f"trailing00={trailing_zero}, channel={ch}, "
                f"PC targets={pc_targets}, SysEx targets={sysex_targets}, "
                f"listening on inputs={self.midi.open_input_names()}"
            )
            for msg_bytes in pc_msgs:
                self._log(f"TX PC-prep: {hex_line(msg_bytes)}")
                sent = self.midi.send_to(msg_bytes, pc_targets, gap_s=0.0)
            ...
            sent = self.midi.send_to(sysex, sysex_targets)
            self._log(f"TX SysEx sent on {len(sent)} output(s): {sent}")
        except Exception as exc:
            self._log(f"Split PC+Current failed: {exc}")
            messagebox.showerror("Split PC+Current", str(exc))

    def request_slot_via_pc_port1(self) -> None:
        self.request_slot_via_pc("XD Port 1")

    def request_slot_via_pc_port2(self) -> None:
        self.request_slot_via_pc("XD Port 2")

    def request_slot_via_pc_both(self) -> None:
        self.request_slot_via_pc("All XD outputs")

    def request_slot_via_pc(self, target_mode: Optional[str] = None) -> None:
        ...
        v5.5: target_mode can force TX to XD Port 1, XD Port 2, or Both while
        RX remains open on all XD inputs.
        """
        try:
            slot = self.slot_index()
            ch = self.channel()
            ...
            mode = target_mode or self.tx_mode_var.get()
            targets = self._target_output_names_for_mode(mode)
        except Exception as exc:
        ...
        try:
            self._log(
                ...
                f"tx_mode={mode!r}, channel={ch}, targets={targets}, "
                f"listening on inputs={self.midi.open_input_names()}"
            )
            for msg_bytes in pc_msgs:
                self._log(f"TX PC-prep: {hex_line(msg_bytes)}")
