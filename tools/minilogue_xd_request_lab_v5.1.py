...
Safe startup:
- no MIDI enumeration on startup
- no auto-open
- no auto-send
...
class MidiWorker:
    ...
    def open(self, in_name: Optional[str], out_name: Optional[str]) -> None:
        ...
        self.close()
        if in_name:
            self.inport = mido.open_input(in_name, callback=self._on_message)
        if out_name:
    ...
    def send(self, raw: bytes) -> None:
        if self.outport is None:
            raise RuntimeError("MIDI OUT port is not open")
        if mido is None:
            raise RuntimeError(f"mido import failed: {MIDO_IMPORT_ERROR}")
    ...
    def _on_message(self, msg) -> None:
        try:
            ...
            raw = bytes()
        self.incoming.put(("midi", (getattr(msg, "type", "unknown"), raw)))


class RequestLab:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("minilogue xd Request Lab - Safe Mode")
        ...
        self._build_gui()
        self._log("Safe startup active. Click Refresh Ports only when ready.")
    ...
    def _build_gui(self) -> None:
        ...
        ttk.Button(top, text="Open Ports", command=self.open_ports).grid(row=0, column=5, padx=4)
        ttk.Button(top, text="Close Ports", command=self.close_ports).grid(row=0, column=6, padx=4)
        ttk.Label(top, textvariable=self.status_var).grid(row=1, column=0, columnspan=7, padx=4, pady=4, sticky="ew")
        ...
        ttk.Button(req, text="Full Bank Sequential", command=self.request_full_bank).grid(row=2, column=3, padx=4, pady=6)
        ttk.Button(req, text="Listen Manual Dump", command=self.listen_manual).grid(row=2, column=5, padx=4, pady=6)
        ttk.Button(req, text="Cancel", command=self.cancel_operation).grid(row=2, column=6, padx=4, pady=6)
    ...
    def refresh_ports(self) -> None:
        if mido is None:
            ...
            return
        try:
            self.input_ports = list(mido.get_input_names())
            self.output_ports = list(mido.get_output_names())
        ...
        self.input_var.set(self._preferred(self.input_ports, False))
        self.output_var.set(self._preferred(self.output_ports, True))
        self._log(f"Ports refreshed: {len(self.input_ports)} input, {len(self.output_ports)} output")

    @staticmethod
    def _preferred(ports: list[str], output: bool) -> str:
        if not ports:
            return ""
        markers = ("midiout2", " 2", ") 2") if output else ("midiin2", " 2", ") 2")
        for marker in markers:
            for port in ports:
                low = port.lower()
                if "minilogue xd" in low and marker in low:
                    return port
        for port in ports:
            if "minilogue xd" in port.lower():
                return port
        return ports[0]
    ...
    def open_ports(self) -> None:
        try:
            ...
            self._log(f"Open ports failed: {exc}")
            messagebox.showerror("Open ports failed", str(exc))
            return
        self.status_var.set(f"Ports opened: IN={self.input_var.get()!r}, OUT={self.output_var.get()!r}")
    ...
    def _probe_worker(self, slot: int, timeout_s: float) -> None:
    ...
    def request_full_bank(self) -> None:
        ...
        self.operation_thread = threading.Thread(
            ...
            args=(channel, trailing_zero, timeout_s, delay_s),
            daemon=True,
    ...
    def _full_bank_worker(
        ...
        """Sequentially request slots 0..499 and wait for each 0x4C response."""
        received = 0
        timeouts = 0
        mismatches = 0
        ...
        for slot in range(500):
            ...
            # Used by the GUI receive handler to map the next 0x4C to the requested slot.
            self.expected_mode = "bank"
            self.expected_slot = slot

            self.queue.put(("log", f"TX FullBank slot {slot + 1:03d}: {hex_line(raw)}"))
            ...
            if response_slot is None:
                timeouts += 1
                self.queue.put(("log", f"Timeout waiting for 0x4C slot {slot + 1:03d}; continuing."))
            else:
    ...
    def write_bank(self) -> None:
        ...
        if not messagebox.askyesno("Write bank to XD", f"Send {len(records)} program dump(s) sequentially?\n\nThis may overwrite program slots on the device."):
            return
        self.cancel_event.clear()
        self.operation_thread = threading.Thread(target=lambda: self._write_bank_worker(records), daemon=True)
        self.operation_thread.start()

    def _write_bank_worker(self, records: list[ProgramRecord]) -> None:
        sent = 0
        for rec in records:
            if self.cancel_event.is_set():
            ...
            try:
                self.midi.send(rec.raw)
                sent += 1
                self.queue.put(("log", f"WRITE {sent}/{len(records)} slot={rec.display_slot} name={rec.name}"))
    ...
    def process_queue(self) -> None:
        while True:
            try:
                ...
                break
            if kind == "midi":
                msg_type, raw = payload
                if raw and raw[0] == 0xF0:
                    self._handle_sysex(raw, source="live")
                elif msg_type not in REALTIME_TYPES:
                    self._log(f"MIDI {msg_type}: {hex_line(raw)}")
            elif kind == "log":
    ...
    def _handle_sysex(self, raw: bytes, source: str) -> None:
        ...
        if cmd in (CMD_RX_PROGRAM, CMD_RX_CURRENT):
            ...
            if cmd == CMD_RX_PROGRAM and slot is not None:
