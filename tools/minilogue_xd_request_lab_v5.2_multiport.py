#!/usr/bin/env python3
"""
minilogue_xd_request_lab_v5.2_multiport.py
...
class RequestLab:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("minilogue xd Request Lab v5.2 - Multiport")
        ...
        self._build_gui()
        self._log("v5.2: startup will auto-refresh and open all minilogue xd ports after GUI init.")
    ...
    def _build_gui(self) -> None:
        ...
        ttk.Button(req, text="Full Bank Sequential", command=self.request_full_bank).grid(row=2, column=4, padx=4, pady=6)
    ...
    def _full_bank_worker(
        ...
        for slot in range(500):
            ...
            try:
                sent = self.midi.send_to(raw, target_names)
                self.queue.put(("log", f"TX FullBank sent on outputs: {sent}"))
            except Exception as exc:
